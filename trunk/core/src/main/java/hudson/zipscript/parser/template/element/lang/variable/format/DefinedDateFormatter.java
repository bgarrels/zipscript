...
public class DefinedDateFormatter implements Formatter {
	...
	public String format(Object object, ZSContext context) throws Exception {
		if (null == context.getLocale() || null == this.locale
			...
			DateFormat formatter = (DateFormat) localeFormatters.get(context.getLocale());
			if (null == formatter) formatter = getDateFormatter(
					format, context.getLocale());
		...
		if (null == p2) {
			...
			if (p1.length() == 1) {
				...
				return DateFormat.getDateTimeInstance(
						normalizeStyle(p1), normalizeStyle(p2), locale);
			}
		}
	}
	...
	private int normalizeStyle (String style) {
		if (style.equals("short"))
			...
			throw new ExecutionException("Unknown date format '" + style + "'", null);
		}
	}
}
