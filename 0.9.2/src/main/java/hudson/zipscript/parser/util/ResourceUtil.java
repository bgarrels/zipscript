...
public class ResourceUtil {
	 ...
	 */
	public static TemplateResource loadTemplate (
			ParseParameters parseParameters, ResourceContainer resourceContainer, Resource resource)
	throws ParseException {
		return loadTemplate(null, parseParameters, resourceContainer, resource, TextDefaultElementFactory.INSTANCE);
	}
	...
	private static TemplateResource loadTemplate (
			PatternMatcher[] patternMatchers, ParseParameters parseParameters, ResourceContainer resourceContainer,
			Resource resource, DefaultElementFactory defaultElementFactory)
	throws ParseException {
		...
		if (null != resourceContainer.getComponents()) {
			pr = ExpressionParser.getInstance().parse(
					contents, resourceContainer.getComponents(), defaultElementFactory, 0, resourceContainer);
		}
		else {
			pr = ExpressionParser.getInstance().parse(
